...
const { seedTestDataToSqlite } = require('../test/seedTestData')
const isInsertUpdateDeleteQuery = require('./utils/isInsertUpdateDeleteQuery')
...
function executeQuery (query, values, cb) {
  ...
  return db.serialize(function () {
    if (isInsertUpdateDeleteQuery(query)) {
      db.run(query, values, function (err) {
        if (err) return cb(err)
        cb(null, { message: 'Query executed successfully', changedRows: this.changes })
      })
    } else {
